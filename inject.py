...
def log(value, status='INFO', color='[R1]'):
    value = colors('{color}''[CC]''[{time}] [P1]:: {color}{status} [P1]:: {color}{value}[CC]'.format(
        time=datetime.datetime.now().strftime('LAMRA STUDIO'),
        value=value,
        color=color,
        status=status
    ))
...
class inject(object):
    ...
    def start(self):
        try:
            ...
            frontend_domains = open(real_path('/lamra.txt')).readlines()
            frontend_domains = filter_array(frontend_domains)
            if len(frontend_domains) == 0:
                self.log('Frontend Domains not found. Please check lamra.txt', color='[Y1]')
                return
...
class domain_fronting(threading.Thread):
    ...
    def run(self):
        try:
            ...
            self.log('MENGAMBIL JARINGAN...!'.format(self.proxy_host, self.proxy_port))
            ...
            self.log('HAMPIR CONNECT BOS...!''[CC]'.format(self.proxy_host, self.proxy_port), color='[CC]')
        except OSError:
...
def main():
    print(colors('\n'.join([
        ...
        '[G1]TELKOMSEL 2019',
        'PLEASE SUBSCRIBE LIKE COMENT & SHARE',
        '',
    ])))
